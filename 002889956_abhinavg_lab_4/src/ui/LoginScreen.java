    ...
    private void btnLoginActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLoginActionPerformed
        ...
        if(selectedPanel.getClass() == SupplierWorkAreaJPanel.class) {
            if(selectedSupplier == null) {
                JOptionPane.showMessageDialog(selectedPanel,"Please select a supplier to login under supplier role");
                return;
            } else {
...
//        Supplier[] items = new Supplier[supplierDirectory.getSupplierList().size()];
//        for(int i =0 ; i<supplierDirectory.getSupplierList().size();i++) {
//            items[i] = supplierDirectory.getSupplierList().get(i);
//        }
//        cmbSuppliers.setModel(new DefaultComboBoxModel(items));  
    ...
    private void updateSupplierVisibility() {
        if(cmbRoles.getSelectedItem() == null) return;
        if(cmbRoles.getSelectedItem().toString() == "Administrator"){
            lblSupplier.setVisible(false);
            cmbSuppliers.setVisible(false);
        } else {
